...
//    const char fmt_str11[] = "xdp_redirect_func : %c \n";
//    bpf_trace_printk(fmt_str11, sizeof(fmt_str11), "1");
	/* Parse Ethernet and IP/IPv6 headers */
	eth_type = parse_ethhdr(&nh, data_end, &eth);
	if (eth_type == -1)
		goto out;
...
//    const char fmt_str12[] = "xdp_redirect_map : %c \n";
//    bpf_trace_printk(fmt_str12, sizeof(fmt_str12), "1");
	/* Parse Ethernet and IP/IPv6 headers */
	eth_type = parse_ethhdr(&nh, data_end, &eth);
	if (eth_type == -1)
		goto out;
...
int xdp_router_func(struct xdp_md *ctx)
{
    ...
    if (h_proto == bpf_htons(ETH_P_IP)) {
		...
		if (iph->ttl <= 1)
			goto out;
/*
    long unsigned int es_source = 33558700;

    if (iph->saddr == 184549386){
        iph->saddr = es_source;
    }
    if (iph->daddr == 184549386){
        iph->daddr = es_source;
    }
*/
     ...
     //  const char fmt_str[] = "Source ip: %lu \n";

     //  bpf_trace_printk(fmt_str, sizeof(fmt_str), iph->saddr);
